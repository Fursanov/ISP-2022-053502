...
def inp_n ():
    ...
    while int(number) > len(max_string) or int(number) <= 0:
        print("ошибка ввода: текст не имеет таких больших n-грамм или введён 0")
        print("Введите n, количество букв n-грамм")
        return 0
    return int(number)
...
if len(literalcounter)%2==0:
    print("Медианное количество слов в предложении: ",(literalcounter[int(len(literalcounter)/2)]+literalcounter[int(len(literalcounter)/2)-1])/2)
else:
    print("Медианное количество слов в предложении: ",literalcounter[int(len(literalcounter)/2)])
