...
def isdict(a):
    """Функция десериализует объект из словаря."""
    a = a[1:len(a) - 1]
    ret_obj = {}
    recursiv = 0
    lists = []
...
def islist(a):
    """Функция десериализует объект из списка."""
    a = a[1:len(a) - 1]
    b = a
    ret_obj = list()
    recursiv = 0
...
def is_digit(string):
    """Функция проверяет строку на число."""
    if string.isdigit():
        return True
    else:
        try:
...
def dumps(a, rec=-100):
    """Функция сериализации обЪекта в строку."""
    """принимает объект и флаг новой строки."""
    if(rec > 0):
        Next = 1
    elif(rec == 0):
        Next = 1
...
def loads(a, b=0):
    """Функция десериализации обЪекта из строки, принимает строку."""
    if isinstance(a, str):
        a = a.replace('\n', '').replace('\t', '').replace('\\', '')
    else:
        return a
...
def serialize_iterable(obj):
    """Функция распаковывает глобальные переменные."""
...
def get_global_vars(func):
    """Функция возврящает глобальные переменные для заданной функции."""
    globs = {}
...
def is_iterable(obj):
    """Функция проверяет итерабельный ли объект."""
    return getattr(obj, "__iter__", None) is not None


def serialize_function(obj, rec=-100):
    """Функция сериализации функции, принимает функцию и флаг новой строки."""
...
def deserialize_function(src):
    """Функция десериализации функции, принимает словарь."""
    src = loads(src)
    ...
    for i in res.__globals__:
        try:
            ...
            res.__globals__[i] = res.__globals__[i][1:len(
                res.__globals__[i]) - 1]
    return res


def is_function(obj):
    """Функция проверяет является ли объект функцией."""
    return (inspect.isfunction(obj) or
            inspect.ismethod(obj) or isinstance(obj, LambdaType))


def is_class(obj):
    """Функция проверяет является ли объект классом."""
    return inspect.isclass(obj)


def collect_funcs(obj, is_visited):
    """Функция для сборки функции."""
...
def serialize_class(obj):
    """Функция сериализации класса, принимает класс."""
...
def deserialize_class(src):
    """Функция десериализации класса, принимает словарь."""
    a = 0
    for i in src:
        a += 1
        if a % 2 == 0:
...
def dump(a, file, rec=-100):
    """Функция сериализации обЪекта в файл."""
    """принимает объект, имя файла и флаг новой строки."""
    with open(file, 'w') as outfile:
        outfile.write(dumps(a, rec))


def load(file, b=0):
    """Функция десериализации файла в объект, принимает имя файла."""
    with open(file, 'r') as outfile:
        return loads(outfile.read(), b)
